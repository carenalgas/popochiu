...
class GDScriptParser:
    ...
    def parse_content(self, content: str, file_path: str = "") -> Optional[ClassInfo]:
        ...
        while i < len(lines):
            ...
            # Clear doc lines if we hit a non-doc line that's not empty
...
def parse_directory(directory: Path, recursive: bool = True) -> list[ClassInfo]:
...
if __name__ == "__main__":
    ...
    if path.is_file():
        ...
        for cls in classes:
            print(f"  - {cls.name} (extends {cls.extends})")
    else:
