...
Importers are available for [Characters](./PopochiuCharacter.md), [Rooms](./PopochiuRoom.md), and [Inventory items](./PopochiuInventoryItem.md), allowing an almost one-click creation of the complete set of graphical assets representing the game world.
...
* For **Rooms**, every tag represents a [Prop](./PopochiuProp.md) and for each tag, a new prop with its own animation is created and populated by the appropriate animation (or sprite). Background and foreground elements are props, of course, and it will be possible to configure each prop visibility and clickability before importing starts.
* For **Inventory Items**, every tag in a single source file represents an [Inventory items](./PopochiuInventoryItem.md) that - despite they are usually represented by a still image - can also be animated. Having the whole inventory in a single file is very convenient for consistency and easier file management.
...
First notable thing is, a single file contains all animations for the character, identified by tags (_1_). Each tag will be used to create a single animation, named after the tag, for the Character.
...
The very same concept apply to Inventory Items too. Here is an example source file:
...
For characters, the engine will automatically recognize the following reserved names and use them properly: `idle`, `walk`, `talk`, `grab`. Naming animations like this, the engine will for granted that the character is facing right and it will be flipped to the left and used for up and down directions too, unless you don't provide...
...
!!! note
    ...
    If you open a proper target scene and the dock is not shown, maybe the functionality is disabled. Chech that `Popochiu -> Importer -> Enable Aseprite Importer` option is flagged in the **Editor Settings**.
...
For each tag found, three flags are available. Two are commons to all the importers (from right to left):
...
Since the list can get pretty long (in this case, a scrollbar is shown, as in the example image), a header is available (_2_) with some specific function:

* **A fulltext filter** that highlights only the tags containing the string in any position (case-insensitive, no minimum lenght).
* **Bulk buttons** to toggle the state of every tag in the list. This button has three states:
  * It's on when everything is on. Clicking it, switch everything to off.
  * It's off when everything is off. Clicking it, switch everything to on.
  * It's in a "dirty" state when you have a mixed situation. In that case, clicking it will set evething back to the default state (depending on the column). Don't worry, a confirmation dialog is displayed, before you wipe all your precious settings!
...
The **Character** importer finally, only have the basic flags (_1_), because the animation to play is selected and started by the engine:
...
All these preferences are automatically saved separaterly for each scene (globally for the inventory), so they are persisted between sessions, and different for each single room or character.  
If for some reason you want to completely reset an Importer anew, just clich the **Reset preferences** icon (_5_) at the very bottom (confirmation required).

### Importing animations

Finally, if everything is set up correctly, you can hit the **Import** button (_5_). The importer will do its magic (beware: very large files with hundreds to thousands of frames will take a while to load, manly due to Aseprite exporting them).  
...
As for inventory items, already existing props animations and sprite get updated, but the rest of the element is untouched.
