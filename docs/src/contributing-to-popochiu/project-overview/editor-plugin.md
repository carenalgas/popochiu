...
The Main and Room tabs use a native Tree control. In the Main tab, each object type (Rooms, Characters, Inventory items, Dialog trees) is a collapsible group item, and each game object is a child item with icon, status tags (main scene, player character, start-with-it), and quick-action buttons (open, script, state, play). Right-clicking an item (or using its three-dots button) opens a context menu with less frequent actions such as setting the main scene, setting the player character, adding the object back to Popochiu, or removing it. The Room tab lists the objects of the currently open room (Props, Hotspots, Walkable areas, Regions, Markers, Characters in room), with quick-action buttons and a context menu to remove objects, plus an "Add character to room" button on the Characters group. The Audio tab still uses the legacy `popochiu_group` and `popochiu_row` components for now and will be migrated to the Tree-based approach in a future iteration (see [#558](https://github.com/carenalgas/popochiu/issues/558)).
