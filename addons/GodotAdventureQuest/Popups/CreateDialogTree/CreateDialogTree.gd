...
const DIALOG_TREE_SCRIPT_TEMPLATE := \
'res://script_templates/DialogTreeTemplate.gd'
...
func set_main_dock(node: PopochiuDock) -> void:
	.set_main_dock(node)
	# Por defecto: res://src/DialogTrees
	_dialog_path_template = _main_dock.dialog_trees_path + '%s/Dialog%s'
...
func create() -> void:
	...
	_main_dock.dir.make_dir(_main_dock.dialog_trees_path + _new_dialog_name)

	# Crear el script del nuevo diálogo ----------------------------------------
	var dialog_template := load(DIALOG_TREE_SCRIPT_TEMPLATE)
	...
	var dialog_resource: DialogTree = DialogTree.new()
	...
	# Agregar el diálogo al Godot Adventure Quest ----------------------------
	var gaq: Node = ResourceLoader.load(_main_dock.GAQ_PATH).instance()
	gaq.dialog_trees.append(ResourceLoader.load(_new_dialog_path + '.tres'))
	...
	_main_dock.add_to_list('dialog_tree', _new_dialog_name)
...
func _update_name(new_text: String) -> void:
	...
	if _name:
		...
		_info.bbcode_text = (
			...
			% [
				_main_dock.dialog_trees_path + _new_dialog_name,
