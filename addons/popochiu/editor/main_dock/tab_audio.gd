...
func _group_audio_cues() -> void:
	...
	for key: String in _groups:
		...
		for resource_path: String in group_data:
			if not FileAccess.file_exists(resource_path):
				entries_to_delete[key].append(resource_path)
				continue
	...
	for key: String in entries_to_delete:
		if entries_to_delete[key].is_empty(): continue
		
		var group: String = _groups[key].array
		var paths: Array = PopochiuResources.get_data_value("audio", group, [])
		
		for resource_path: String in entries_to_delete[key]:
			var audio_cue_name: String = resource_path.get_file().get_basename()
			
			(_groups[key].group as PopochiuGroup).remove_by_name(audio_cue_name)
			
			paths.erase(resource_path)
