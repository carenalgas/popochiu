...
func create(obj_name: String, set_as_main:bool = false) -> PopochiuRoom:
	...
	var room_resource: PopochiuRoomData = load(_obj_path + '_state.gd').new()
	room_resource.script_name = _obj_name
	room_resource.scene = _obj_path + '.tscn'
	room_resource.resource_name = _obj_name
	
	if ResourceSaver.save(room_resource, _obj_path + '.tres') != OK:
	...
	var room_script: Script = load(BASE_SCRIPT_TEMPLATE).duplicate()
	var new_code := room_script.source_code
	
	room_script.source_code = ''
	
	if ResourceSaver.save(room_script, _obj_path + '.gd') != OK:
