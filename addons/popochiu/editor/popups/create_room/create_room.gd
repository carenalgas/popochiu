...
func create() -> void:
	...
	if ResourceSaver.save(state_template, _new_room_path + '_state.gd') != OK:
		push_error('[Popochiu] Could not create room state script: %s' %\
		_new_room_name)
		# TODO: Show feedback in the popup
		return
	
	var room_resource: PopochiuRoomData = load(_new_room_path + '_state.gd').new()
	...
	new_code = new_code.replace(
		'Data = null',
		"Data = preload('room_%s.tres')" % _new_room_name
	)
